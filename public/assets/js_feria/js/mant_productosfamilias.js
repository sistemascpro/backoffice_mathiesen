function Guardar()
{
    ...
    if(Requeridos<=0){
        ...
        $.ajax({
            url: 'mant_productosfamilias_Guardar',
            type: 'POST',
...
function eliminar(id)
{
    ...
    cancelButtonText: `NO`,
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: 'mant_productosfamilias_eliminar',
                type: 'POST',
                headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
                data: {
                    _token : $("#_token").val()
...
function crearEditar(id)
{
    ...
    form.method = "POST";
    form.action = "mant_productosfamilias_crearEditar";
    form.target = '_self';
...
{
    $.ajax({
        url: 'mant_productosfamilias_updateEstado',
        type: 'POST',
        headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
        data: {
            _token : $("#_token").val()
