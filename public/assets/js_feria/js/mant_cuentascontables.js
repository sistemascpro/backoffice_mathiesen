function Guardar()
{
    var Requeridos = ValidarRequeridos('CuentaContableRequired');

    if(Requeridos<=0){
        var formInfo = new FormData($("#FormGuardar")[0]);
        $.ajax({
            url: 'mant_cuentascontables_Guardar',
            type: 'POST',
...
function eliminar(id)
{
    ...
    cancelButtonText: `NO`,
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: 'mant_cuentascontables_eliminar',
                type: 'POST',
                headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
                data: {
                    _token : $("#_token").val()
...
function crearEditar(id)
{
    ...
    form.method = "POST";
    form.action = "mant_cuentascontables_crearEditar";
    form.target = '_self';
...
{
    $.ajax({
        url: 'mant_cuentascontables_updateEstado',
        type: 'POST',
        headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
        data: {
            _token : $("#_token").val()
