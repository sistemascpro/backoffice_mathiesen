function Guardar()
{
    ...
    if(Requeridos<=0){
        ...
        $.ajax({
            url: 'mant_productosclases_Guardar',
            type: 'POST',
...
function eliminar(id)
{
    ...
    cancelButtonText: `NO`,
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: 'mant_productosclases_eliminar',
                type: 'POST',
                headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
                data: {
                    _token : $("#_token").val()
...
function crearEditar(id)
{
    ...
    form.method = "POST";
    form.action = "mant_productosclases_crearEditar";
    form.target = '_self';
...
{
    $.ajax({
        url: 'mant_productosclases_updateEstado',
        type: 'POST',
        headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
        data: {
            _token : $("#_token").val()
...
function actualizarEstado(id, estado)
{
    ...
    {
        var titulo = 'ACTIVAR?';
        var texto = 'LA CLASE PODRA SER ASCOCIADA A PRODUCTOS!';
    }
    else
    {
        var titulo = 'BLOQUEAR?';
        var texto = 'LA CLASE NO PODRA SER ASOCIADA A PRODUCTOS!';
    }
